...
Uses OSM IDs for start/end points and generates two plots
in a single window:
1. A map coloring all edges by their predicted weight Car flow.
...
# --- Pathfinding Params (Use your own OSM IDs) ---
START_OSMID = "9293223530"  # (Example: Change this)
END_OSMID = "4038648450"    # (Example: Change this)
...
def plot_predicted_weights(ax, pos_dict, edge_index, predicted_weights, title):
    ...
    cbar.set_label('Predicted Car flow')
...
def plot_path(ax, pos_dict, G, path, title="Graph with Shortest Path"):
    ...
    nx.draw_networkx_edges(G, pos_dict, ax=ax, width=0.4, edge_color='black', alpha=0.7, arrows=False)
...
def main_pathfinder():
    ...
    try:
        ...
        first_val_snap = val_snaps[2]
        expected_edges = edge_index.shape[1]
        edge_attr_snapshot, used_key = extract_edge_attr(first_val_snap, expected_edges)
    ...
    fig.canvas.manager.set_window_title('A* Pathfinding with Predicted Weights')
    
    plot_predicted_weights(
        axes[0], pos_dict, edge_index, predicted_weights,
        title="Map 1: All Edges Colored by Predicted Weight"
    )

    path_astar = None 
    path_title = "Map 2: Pathfinding"
    ...
    try:
        ...
        print(f"A* Path found: {len(path_astar)} nodes, Predicted Cost: {cost_astar:.2f}")
        path_title = f"Map 2: A* Path ({START_OSMID} to {END_OSMID})"
        ...
        print("Please check your START_OSMID and END_OSMID values.")
